describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let jsonV = await innerText.jsonValue();
    expect(jsonV).toBe("Remove from Cart");
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await btn.getProperty("innerText");
      let jsonV = await innerText.jsonValue();
      if (jsonV.localeCompare("Add to Cart")==0) {
        await btn.click();
      }
    }
    let count = await page.$("#cart-count");
    let text = await count.getProperty("innerText");
    let jsonV = await text.jsonValue();
    expect(jsonV).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await btn.getProperty("innerText");
      let jsonV = await innerText.jsonValue();
      expect(jsonV.localeCompare("Remove from Cart")==1);
    }
    let count = await page.$("#cart-count");
    let text = await count.getProperty("innerText");
    let jsonV = await text.jsonValue();
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await btn.getProperty("innerText");
      let jsonV = await innerText.jsonValue();
      if (jsonV.localeCompare("Remove from Cart")==0) {
        await btn.click();
      }
    }
    let count = await page.$("#cart-count");
